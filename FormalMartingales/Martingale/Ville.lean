...
* `FormalMartingales.Supermartingale.expected_stoppedValue_le_start`: for a supermartingale, the
  expected stopped value at a bounded stopping time is at most the initial expectation.
* `FormalMartingales.ville_maximal_ineq`: finite-horizon Ville's inequality.
* `FormalMartingales.ville_maximal_ineq_exists_le`: finite-horizon Ville's inequality with the
  crossing event stated as `∃ k ≤ n`.
* `FormalMartingales.ville_maximal_ineq_of_integral_le_one`,
  `FormalMartingales.ville_maximal_ineq_exists_le_of_integral_le_one`,
  `FormalMartingales.ville_inequality_of_integral_le_one`: the probability-normalized `α` forms.
* `FormalMartingales.ville_inequality`: anytime Ville's inequality.
...
/-- First half of finite-time Ville: on the maximal-crossing event, the threshold times the event
mass is bounded by the integral of the value at the hitting time. Supermartingale analogue of
